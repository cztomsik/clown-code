...
- **Conversation Persistence**: Snapshots (messages, total tokens) can be saved to `session-YYYY-MM-DD HH:MM:SS UTC.json` and reloaded. The `/continue` command auto-loads the most recent session. JSON key order is preserved to stay byte-compatible with the Zig original.

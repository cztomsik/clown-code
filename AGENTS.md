...
| `src/tui.rs` | Terminal UI (ratatui). Event loop (render, key input, idle), message display, footer (status row + input box), command handling (`/exit`, `/quit`, `/stop`, `/clear`, `/clear-tools`, `/compact`, `/init`, `/retry`, `/retry-turn`, `/undo`, `/save`, `/load <file>`, `/continue`; the argument is the entire rest of the line, so filenames with spaces work), and scrollback. |
| `src/model.rs` | Core `Clown` struct — manages the AI agent lifecycle. Agent loop (`Agent`, which owns the client, toolbox, and message history), system prompt loading, conversation management, snapshot save/load (JSON), and the worker thread (mpsc-based). Ctrl-C handling: stop the worker; double Ctrl-C exits. |
...
| `src/llm.rs` | The LLM layer — OpenAI-compatible chat types, blocking HTTP client (`reqwest`), and the tool registry/`Toolbox`. Wire format is a stable contract: only `null` optionals are omitted (`skip_serializing_if`) and field order is preserved. |

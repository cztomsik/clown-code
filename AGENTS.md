...
| `src/tui.rs` | Terminal UI (ratatui). Event loop (render, key input, idle), message display, footer (status row + input box), command handling (`/exit`, `/quit`, `/stop`, `/clear`, `/clear-tools`, `/compact`, `/init`, `/retry`, `/undo`, `/save`, `/load <file>`, `/continue`; the argument is the entire rest of the line, so filenames with spaces work), and scrollback. |

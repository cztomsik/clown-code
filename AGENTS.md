...
| `src/tools.rs` | All AI agent tools: `read_file`, `write_file`, `edit_file`, `run_command`, `load_skill`. Each tool has typed args and doc comments (surfaced to the model). The built-in `/init` skill is `BUILTIN_INIT` (`src/skills/init.md`). Tool names are snake_case. |
